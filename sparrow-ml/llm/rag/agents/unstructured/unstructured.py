...
class UnstructuredPipeline(Pipeline):
    def run_pipeline(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            temp_output_dir = (
                os.path.join(temp_dir, output_dir) if debug is False else output_dir
            )
            ...
            vectorstore, embedding_model = self.invoke_pipeline_step(
                lambda: self.build_vector_store(
                    weaviate_url, embedding_model_name, device, files, debug
                ),
        ...
        llm = self.invoke_pipeline_step(
            lambda: Ollama(
                model=cfg.LLM_UNSTRUCTURED, base_url=cfg.BASE_URL_UNSTRUCTURED
            ),
            "Initializing Ollama...",
            local,
        )
        ...
        raw_result, similar_docs = self.invoke_pipeline_step(
            lambda: self.question_answer(
                query, vectorstore, embedding_model, device, llm
            ),
            "Answering question...",
            local,
        )
    ...
    def process_files(self, temp_output_dir, temp_input_dir):
        self.process_local(
            output_dir=temp_output_dir, num_processes=2, input_path=temp_input_dir
        )
        files = self.get_result_files(temp_output_dir)
        return files

    def build_vector_store(
        self, weaviate_url, embedding_model_name, device, files, debug
    ):
    ...
    def count_documents(self, client: weaviate.Client) -> Dict:
        response = client.query.aggregate("Doc").with_meta_count().do()
        count = response
        return count
    ...
    def get_chunks(
        ...
        for element in elements:
            if type(element.metadata.data_source) is not DataSourceMetadata:
    ...
    def add_data_to_weaviate(
        ...
        for filename in files:
            try:
                ...
                chunks, embeddings = self.get_chunks(
                    elements,
                    embedding_model,
                    device,
                    chunk_under_n_chars,
    ...
    def question_answer(
        self, question: str, vectorstore: Weaviate, embedding_model, device, llm
    ):
