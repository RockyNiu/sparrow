...
import instructor
from .helpers.instructor_helper import (
    execute_sparrow_processor,
    merge_dicts,
    track_query_output,
)
...
class InstructorPipeline(Pipeline):
    def run_pipeline(
        ...
        pdf_split_output_dir = (
            None
            if cfg.PDF_SPLIT_OUTPUT_DIR_INSTRUCTOR == ""
            else cfg.PDF_SPLIT_OUTPUT_DIR_INSTRUCTOR
        )
        ...
        if validate_options:
            if options and "tables" in options:
                ...
                num_pages, output_files, temp_dir = pdf_optimizer.split_pdf_to_pages(
                    file_path, pdf_split_output_dir, pdf_convert_to_images
                )
                ...
                query_inputs_form, query_types_form = self.filter_fields_query(
                    query_inputs, query_types, "form"
                )

                for i, page in enumerate(output_files):
                    content, table_contents = execute_sparrow_processor(
                        options, page, strategy, model_name, local, debug
                    )
                    ...
                    if len(query_inputs_form) > 0:
                        ...
                        query_inputs_form, query_types_form = track_query_output(
                            query_inputs_form, answer_form, query_types_form
                        )
                        if debug:
                    ...
                    if table_contents is not None:
                        query_targets, query_targets_types = self.filter_fields_query(
                            query_inputs, query_types, "table"
                        )
                        extractor = HTMLExtractor()

                        answer_table, targets_unprocessed = extractor.read_data(
                            query_targets,
                            table_contents,
                    ...
                    if num_pages > 1:
                        ...
                        answer_current_page = add_answer_page(
                            {}, "page" + str(i + 1), answer_current
                        )
                ...
                content, table_content = processor.extract_data(
                    file_path, strategy, model_name, None, local, debug
                )
                answer = self.execute(
                    query_inputs, query_types, content, query, "all", debug, local
                )
        else:
            raise ValueError(
    ...
    def build_response_class(self, query_inputs, query_types_as_strings):
        ...
        query_types_as_strings = [
            s.replace("Array", "List") for s in query_types_as_strings
        ]
    ...
    def validate_options(self, options: List[str]) -> bool:
        ...
        valid_combinations = [["tables", "unstructured"], ["tables", "markdown"]]

        # Check for valid combinations or empty list
        if not options:  # Valid if no options are provided
            return True
        if sorted(options) in (
            sorted(combination) for combination in valid_combinations
        ):
            return True
        return False
