...
class LlamaIndexPipeline(Pipeline):
    ...
    def run_pipeline(  # type: ignore
        ...
        rag_chain = self.build_rag_pipeline(
            query_inputs, query_types, index_name, debug, local
        )
    ...
    def build_rag_pipeline(self, query_inputs, query_types, index_name, debug, local):
        ...
        embeddings = self.invoke_pipeline_step(
            lambda: self.load_embedding_model(model_name=cfg.EMBEDDINGS),
        ...
        index = self.invoke_pipeline_step(
            lambda: self.build_index(
                cfg.CHUNK_SIZE, llm, embeddings, client, index_name
            ),
            "Building index...",
            local,
        )
        ...
        query_engine = self.invoke_pipeline_step(
            lambda: index.as_query_engine(
                streaming=False, output_cls=ResponseModel, response_mode="compact"
            ),
    ...
    def build_response_class(
        self,
        query_inputs: list[str],
        query_types_as_strings: list[str],
    ) -> type[BaseModel]:
    ...
    def build_index(
        self, chunk_size, llm, embed_model, weaviate_client, index_name
    ) -> VectorStoreIndex:
