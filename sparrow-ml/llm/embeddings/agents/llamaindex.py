...
class LlamaIndexIngest(Ingest):
    def run_ingest(self, payload: str, file_path: str, index_name: str) -> None:
        ...
        client = self.invoke_pipeline_step(
            lambda: weaviate.Client(cfg.WEAVIATE_URL), "Connecting to Weaviate..."
        )

        documents = self.invoke_pipeline_step(
            lambda: self.load_documents(file_path), "Loading documents..."
        )

        embeddings = self.invoke_pipeline_step(
            lambda: self.load_embedding_model(cfg.EMBEDDINGS),
            "Loading embedding model...",
        )

        self.invoke_pipeline_step(
            lambda: self.build_index(
                client, embeddings, documents, index_name, cfg.CHUNK_SIZE
            ),
            "Building index...",
        )
    ...
    def load_documents(self, file_path):
        documents = SimpleDirectoryReader(
            input_files=[file_path], required_exts=[".pdf", ".PDF"]
        ).load_data()
    ...
    def build_index(
        self, weaviate_client, embed_model, documents, index_name, chunk_size
    ):
